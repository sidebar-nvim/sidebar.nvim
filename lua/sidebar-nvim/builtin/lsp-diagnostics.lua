...
local function get_diagnostics(ctx)
    ...
    for bufnr, buffer_diagnostics in pairs(all_diagnostics) do
        if open_bufs[bufnr] ~= nil and vim.api.nvim_buf_is_loaded(bufnr) then
            ...
            for _, diag in pairs(buffer_diagnostics) do
                ...
                if not use_icons then icon = level end
    ...
    local previous_state = vim.tbl_map(function(group) return group.is_closed end, loclist.groups)
...
return {
    title = "Diagnostics",
    icon = "📄",
    draw = function(ctx) return get_diagnostics(ctx) end,
    highlights = {
