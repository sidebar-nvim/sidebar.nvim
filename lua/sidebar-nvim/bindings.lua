...
M.State = {
    ...
    view_bindings = {["q"] = function() require("sidebar-nvim").close() end}
...
local function on_keypress_section(key, section_match, bindings)
    ...
    if section_match == nil then return false end
...
local function on_keypress_view(key, binding) execute_binding(key, binding) end

function M.on_keypress(key, section_index)
    local section_bindings = M.State.section_bindings[key]

    if section_bindings ~= nil then if on_keypress_section(key, section_index, section_bindings) then return end end

    local view_bindings = M.State.view_bindings[key]

    if view_bindings ~= nil then on_keypress_view(key, view_bindings) end
end
