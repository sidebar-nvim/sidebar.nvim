...
local function expand_section_lines(section_lines, lines_offset)
    ...
    if type(section_lines) == "string" then section_lines = vim.split(section_lines, "\n") end

    -- we must offset the hl lines so it matches the current section position
    for _, hl_entry in ipairs(section_hl) do hl_entry[2] = hl_entry[2] + lines_offset end
...
local function build_section_title(section)
    local icon = "#"
    if section.icon ~= nil then icon = section.icon end
...
local function get_lines_and_hl(sections_data)
    ...
    for _, data in pairs(sections_data) do
        ...
        for _, line in ipairs(section_lines) do table.insert(lines, line) end

        for _, hl_entry in ipairs(section_hl or {}) do table.insert(hl, hl_entry) end
...
function M.draw(sections_data)
    ...
    local cursor
    if view.win_open() then cursor = api.nvim_win_get_cursor(view.get_winnr()) end
    ...
    if cursor and #lines >= cursor[1] then api.nvim_win_set_cursor(view.get_winnr(), cursor) end
    if cursor then api.nvim_win_set_option(view.get_winnr(), 'wrap', false) end
