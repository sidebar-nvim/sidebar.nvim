...
local function find_rogue_buffer()
    for _, v in ipairs(a.nvim_list_bufs()) do if string.match(vim.fn.bufname(v), "^SidebarNvim_.*") then return v end end
    return nil
end

---Check if the tree buffer is valid and loaded.
---@return boolean
local function is_buf_valid() return a.nvim_buf_is_valid(M.View.bufnr) and a.nvim_buf_is_loaded(M.View.bufnr) end
...
local function generate_buffer_name() return "SidebarNvim_" .. math.random(1000000) end
...
function M.win_open(opts)
    if opts and opts.any_tabpage then
        for _, v in pairs(M.View.tabpages) do if a.nvim_win_is_valid(v.winnr) then return true end end
        return false
    else
...
function M.set_cursor(opts) if M.win_open() then pcall(a.nvim_win_set_cursor, M.get_winnr(), opts) end end
...
local function get_width()
    if type(M.View.width) == "number" then return M.View.width end
...
function M.open(options)
    ...
    vim.cmd("buffer " .. M.View.bufnr)
    for k, v in pairs(M.View.winopts) do set_local(k, v) end
    vim.cmd(":wincmd =")
    if not options.focus then vim.cmd("wincmd p") end
end
...
function M.get_winnr(tabpage)
    ...
    if tabinfo ~= nil then return tabinfo.winnr end
end
