...
local function _redraw()
    ...
    if view.win_open() then M.State.section_line_indexes = renderer.draw(updater.sections_data) end
end
...
local function _start_timer(should_delay)
    ...
    local delay = 100
    if should_delay then delay = config.update_interval end

    -- wait `delay`ms and then repeats every `config.update_interval`ms
    M.timer:start(delay, config.update_interval, vim.schedule_wrap(function() loop() end))
end
...
function M.open(opts) view.open(opts or {focus = false}) end
...
local function get_start_line(content_only, indexes)
    if content_only then return indexes.content_start end

    return indexes.section_start
end

local function get_end_line(content_only, indexes)
    if content_only then return indexes.content_start + indexes.content_length end
